...
public class Main {
    public static void main(String[] args) {
        ...
        for (String dates : queryDates) {
            date_string = "temporal_" + dates.split(" ")[0];
            ...
            for (int i = 0; i < queryWordsTwo.size(); i += 2) {
                ...
                WriteResults(queryWordsString, date_string);

                startTime = System.currentTimeMillis();
                nodeCount = traverseStart(model, queryWordsSet, dates, date_string);
    ...
    public static List<YagoNode> getTemporalNodes(Model model, String date) {
        ...
        String dateString = "?date >= \"" + date.split(" ")[0] + "\"^^xsd:date && ?date <= \"" + date.split(" ")[1]
                + "\"^^xsd:date";

        String queryString = "PREFIX xsd:<http://www.w3.org/2001/XMLSchema#> "
                + "PREFIX yago:<http://yago-knowledge.org/resource/> "
                + "PREFIX rdfs:<http://www.w3.org/2000/01/rdf-schema> " + "SELECT DISTINCT ?s " + "WHERE {"
                + "VALUES ?p {yago:wasCreatedOnDate yago:wasDestroyedOnDate} " + "?s ?p ?date . " + "FILTER ("
                + dateString + " && ?p != rdfs:label)" + "}";
    ...
    public static int traverseStart(Model model, HashSet<String> queryWords, String dates, String date_string) {
        // Use a list in parent node for all children that gets a hit.
        // group all hit Children
        // while hitListChildren is not empty: select child, check for hit children in
        // child, ... something, its dinnertime...
        // use node level for tabs when printing, displaying inheratance

        List<YagoNode> roots = getTemporalNodes(model, dates);
        ...
        WriteResults("\nRoot nodes Found: " + roots.size(), date_string);
        for (YagoNode root : roots) {
            ...
            while (!nodes.isEmpty()) {
                ...
                if (node.getTokenList().equals(queryWords))
            ...
            findMinSubgraph(root, queryWords, date_string);
        }
        return nodeCount;
    }
    ...
    public static ArrayList<YagoNode> traverse(Model model, YagoNode yagoNode, HashSet<String> queryWords) {
        ...
        String queryString = "PREFIX rdf:<http://www.w3.org/1999/02/22-rdf-syntax-ns#> "
                + "PREFIX rdfs:<http://www.w3.org/2000/01/rdf-schema> " + "SELECT DISTINCT ?p ?o WHERE { " + "<"
                + entity + "> ?p ?o . " + "FILTER ( ?p != rdf:type && ?p != rdfs:label ) " + "}";
    ...
    public static void findMinSubgraph(YagoNode rootNode, HashSet<String> queryWords, String date_string) {
        ...
        if (rootNode.getTokenList().containsAll(queryWords)) {
            rankSubGraphs(minTree, queryWords, date_string);
        }
    ...
    public static void rankSubGraphs(HashSet<YagoNode> rootNodes, HashSet<String> queryWords, String date_string) {
        double score = 0.0;
        double hitRate = 0.0;
        String results = "\n";
        String scoreString = "";
        ...
        WriteResults(results, date_string);
        ...
        WriteResults(scoreString, date_string);
    }

    public static void WriteResults(String s, String file_name) {
        try {
            Files.write(Paths.get("results/" + file_name + ".txt"), s.getBytes(), StandardOpenOption.CREATE,
